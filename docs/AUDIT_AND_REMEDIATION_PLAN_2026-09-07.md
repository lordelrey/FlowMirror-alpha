...
| **W2** | `world.py`（PATCH，W1 后） | `image_pool_summary(pool, images_root)`；`INVARIANTS["m_tv_arm_carries_images"]`：root 空→skipped(reason)，TV 不在臂集→skipped，否则 `pass = attached>0`；`write_reports` 写 `run_meta["images"]` | `test_images_invariant.py`：手造 state，attached=0 → fail；=3 → pass；root 空 → skipped |
...
14 项全部有业主签字。**执行模型不得改动本表任何一项**；发现数学上不可满足时按 §三 的规矩上报，不得自行改默认值。
...
7. `m_tv_arm_carries_images` 在配图运行上 pass、在无图运行上 skipped 并给出原因
