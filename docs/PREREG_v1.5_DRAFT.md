...
| D31 | 大 V 层 `social_graph.enabled` 默认关：句柄 `"@u" + sha256(agent_id)[:6]`（6 位：400 人撞名概率约 0.5%，5 位约 7%）；`follow_users` schema（只接受当日可见作者的句柄，否则 `unknown_handle` 违规）；「你关注的人昨天」块（≤3 条，t−1）；`st.what == "follow_user"` 事件；`dec.p_follow_users` 只在开启时写入。**2026-09-09 修订（卡 E7/E8A/E8B）**，起因是真模型开启后关注边为 0，实测原因是模型看不到任何句柄：(a) 句柄与粉丝数的渲染**不再受舆论标签约束**——只要该帖 t−1 有评论就渲染摘录，`no_signal` 时表头只报条数、不断言舆论倾向（`climate_for` 的 `min_n=4` 门槛保留不动）；句柄曝光面由 22.4% 升到 41.6%（替身 100 人复测 18.5% → 49.4%）。(b) 热评平手规则由 `agent_id` 升序改为 `sha256(run_tag|t|post_id|agent_id)`，粉丝数仍是主键；理由是粉丝全为 0 时 id 成了唯一有效键，实测只有 id 最小的 60/100 人可能被看见，「谁成名」由编号决定，不构成涌现；改后 78/100 人有机会。(c) 卡片摘录改为**每 agent 各自**：前 2 个位置优先给该 agent 关注对象写的评论（即使全平台前三未选中），标记「（你关注的）」，其余按共享排名填并按作者去重；理由是共享的前三名让关注不产生任何后果，反馈边不存在。(d) 新增平台级「平台推荐关注（按粉丝数）」块，展示 t−1 粉丝最多的 3 个账号（句柄、粉丝数、昨日发言条数，**不含立场**以免诱导），仅当至少一人粉丝 ≥1 时出现，冷启动时不存在。以上四项全部在开启态生效，关闭态提示与事件日志逐字节不变（已对 `runs/out/acc_e6` 三次验证）；开启态 `prompt_sha` 因此变动。零结果措辞：若关注边仍为 0，报告为「在本模型与本窗口下未观察到自发的社交连接形成」，措辞档 `collective_qualitative`。 | 新机制 | E7/E8A/E8B 已落地，待真模型探针 |
